...
export class Garage extends Phaser.Scene {
    ...
    create() {
        ...
        mintCarButton.on('pointerdown', () => { if(window.memberNFT) { return console.log("You already minted NFT")} this.mintNFT() });

        console.log(window.dappContract);
    }

    async mintNFT() {
        try {
          const mintTxn = await window.dappContract.mintDriverNFT();
          await mintTxn.wait();
        } catch (error) {
